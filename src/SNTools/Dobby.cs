...
{
    [DllImport("dobby_x64.dll", EntryPoint = "DobbyHook")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static extern int Hook(nint target, nint detour, ref nint original);

    [DllImport("dobby_x64.dll", EntryPoint = "DobbyDestroy")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static extern int Destroy(nint target);

    public static nint HookAttach(nint target, nint detour)
    {
        nint original = 0;
        return Hook(target, detour, ref original) != 0
